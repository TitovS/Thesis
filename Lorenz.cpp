...

{
    ...
    //Переход в дифференцирование по длине
    double dl = sqrt(pow(m_v [0], 2)+pow(m_v [1], 2)+pow(m_v [2], 2));
      
    m_v [0] = m_v[0]/dl; 
    m_v [1] = m_v[1]/dl; 
    m_v [2] = m_v[2]/dl;
...
void Lorenz::HashFun (double*& _np){
    ...
    int _hp0=_np[0];  // Вопрос первый - побитовые операции невозможны с double. Я пока использую такой метод преобразования. За счет того, что значения дискретизованы я не теряю точность.
    int _hp1=_np[1];
    int _hp2=_np[2];
